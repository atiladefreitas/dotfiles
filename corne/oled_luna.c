...
/* ---- Master info panel: layer + WPM + mods, then Luna at the bottom ---- */
...
static void render_mods_panel(void) {
    uint8_t const mods = get_mods() | get_oneshot_mods();
...
static void render_master_status(void) {
    ...
    // Top: info panel (each oled_write advances the cursor down in 270 rotation)
    oled_set_cursor(0, 0);
    render_layer_name();
    oled_set_cursor(0, 3);
    render_wpm_panel();
    ...
    // Bottom: Luna animation, redrawn on the frame interval
...
bool oled_task_user(void) {
    is_keyboard_master() ? render_master_status() : render_slave_status();
    return false;
}
